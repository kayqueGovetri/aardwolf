...
class RDPIOSettings:
    def __init__(self):
        ...
        # if None: it will be determined automatically. otherwise select one or more from:
        # SUPP_PROTOCOLS.RDP | SUPP_PROTOCOLS.SSL | SUPP_PROTOCOLS.HYBRID_EX
        self.supported_protocols = None

        # Video settings used by both RDP and VNC
        self.video_width = 1024
        self.video_height = 768
        # the rectangle image format which will be sent via the queue
        self.video_out_format = VIDEO_FORMAT.PIL
        ...
        self.performance_flags = (
            ...
            | PERF.DISABLE_THEMING
            | PERF.DISABLE_CURSORSETTINGS
            | PERF.DISABLE_MENUANIMATIONS
            | PERF.DISABLE_FULLWINDOWDRAG
        )

        # Broker / negotiation options
        # Optional RDS load-balance cookie (RDP file: loadbalanceinfo -> Cookie: msts=<value>)
        self.loadbalance_info = None
        # Prefer NLA-only (HYBRID/HYBRID_EX) without also advertising TLS/SSL
        self.prefer_nla_only = False
        # Gate keyboard/mouse until first bitmap update to avoid RemoteApp-only disconnects
        self.allow_input_without_video = False

        # Keyboard settings
        # client_keyboard is used by VNC where the keyboard state must be virtualized client-side
        ...
        # store all incoming clipboard data received from server. Text only
        ...
        # Each value corresponds to an encoding type and only these three are implemented currently.
